...
//----------------------------------------- struct StepperBase
StepperBase::StepperBase(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx, const Doub atoll,
...
StepperBS<D>::StepperBS(VecDoub_IO &yy,VecDoub_IO &dydxx,Doub &xx,
	const Doub atoll,const Doub rtoll, bool dens, PARAMS parr) :
	...
	fsave(IMAXX*(2*IMAXX+1),n),ipoint(IMAXX+1),dens((2*IMAXX+5)*n), par(parr) {
	EPS=numeric_limits<Doub>::epsilon();
	if (dense)
		for (Int i=0;i<IMAXX;i++)
			nseq[i]=4*i+2;			// y esta seq???
	else
		for (Int i=0;i<IMAXX;i++)
			nseq[i]=2*(i+1);		// p.924, eq(17.3.6)
...
void StepperBS<D>::step(const Doub htry, D &derivs) {
	...
	work[0]=0;
	h=htry;
	forward = h>0 ? true : false;		// avanzando o retrocediendo en tiempo
	...
	while (firstk || reject) {
		...
		for (k=0; k<=k_targ+1;k++) {
			...
			if (k != 0) {
				...
				err=0.0;
				for (i=0;i<n;i++) {
					scale[i]=atol+rtol*MAX(abs(ysav[i]),abs(y[i]));	// "scale" different for each component
					err+=SQR((y[i]-table[0][i])/scale[i]);		// eq(17.2.9)
				}
				err=sqrt(err/n);
				Doub expo=1.0/(2*k+1);
...
template class StepperBS<rhs>; // rhs: system of equations I use!
